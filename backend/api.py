...
@app.post("/order/details", response_model=OrderResponse)
async def get_order_details(request: OrderRequest):
    # Mock implementation - always returns the same order code format
    # In a real implementation, you would query a database
    
    # Validate request
    if not request.orderCode:
        raise HTTPException(status_code=400, detail="Order code is required")
    
    # Create the mock response
    modified_code = f"{request.orderCode}*PED*2025"
    
    return OrderResponse(
        orderCode=modified_code,
        orderDetails=f"Details for order {modified_code}",
        status="Processing"
    )