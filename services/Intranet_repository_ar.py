...
class IntranetRepositoryAR:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(IntranetRepositoryAR, cls).__new__(cls)
        return cls._instance

    def __init__(self, index_path="faiss_index_ar"):
        self.index_path = index_path
        self.docs_file = os.path.join("docs", "pedidos_argentina.txt")
    ...
    def create_or_load_faiss_index(self, force_rebuild=False):
        ...
        if IntranetRepositoryAR._vectorstore is not None and not force_rebuild:
            logger.info("Reusing existing FAISS index from memory.")
            return IntranetRepositoryAR._vectorstore

        # If the index exists on disk and we don't need to rebuild, load it
        if os.path.exists(self.index_path) and os.path.isfile(f"{self.index_path}/index_ar.faiss") and not force_rebuild:
            logger.info(f"Loading FAISS index from {self.index_path}")
            try:
                IntranetRepositoryAR._vectorstore = FAISS.load_local(
                    self.index_path,
                    OpenAIEmbeddings(),
        ...
        try:
            ...
            if len(chunks) <= batch_size:
                ...
                IntranetRepositoryAR._vectorstore = FAISS.from_documents(chunks, embeddings)
            else:
                # Create index in batches for larger sets
                logger.info(f"Creating index in batches of {batch_size} chunks")
                # Create first batch
                first_batch = chunks[:batch_size]
                IntranetRepositoryAR._vectorstore = FAISS.from_documents(first_batch, embeddings)
                ...
                for i in range(batch_size, len(chunks), batch_size):
                    ...
                    if batch:
                        IntranetRepositoryAR._vectorstore.add_documents(batch)
            
            # Ensure the directory exists
            os.makedirs(self.index_path, exist_ok=True)
            
            # Save the index
            IntranetRepositoryAR._vectorstore.save_local(self.index_path)
    ...
    def query_document(self, question, k=3):
        ...
        if IntranetRepositoryAR._vectorstore is None:
        ...
        docs = IntranetRepositoryAR._vectorstore.similarity_search(question, k=k)
    ...
    def force_rebuild_index(self):
        ...
        try:
            ...
            index_files = [
                os.path.join(self.index_path, "index_ar.faiss"),
                os.path.join(self.index_path, "index_ar.pkl")
            ]
